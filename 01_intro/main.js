/*
  ...
  这样是不被允许的，如果是兄弟组件使用相同的 props 数据
  其中有一个子组件更改了 props 数据则会导致数据流混乱
...
/*
  ...
  向 vm 身上挂载 $data 代理对象
  好处就是为了避免用户直接操作数据源，防止数据污染

  形成代理对象后，开始对 data 数据绑定响应式监听
  通过 Object.defineProperty 绑定 getter 与 setter 函数
  getter 函数的触发返回当前的属性值，同时触发依赖收集
  setter 函数的触发更新当前的属性值，通知依赖视图更新
  这里 Vue 使用了消息订阅与发布模式
  通过 Dep 类对模板中插值语法引用的属性进行依赖收集
  当数据发生变更时通知对应的回调函数去更新视图

  然后开始初始化 options 中其它的配置
  例如 computed/methods/watch
  ...
  调用 $mount 方法挂载，触发 render 函数

  mounted()
  此时可以通过 this.$refs 访问页面中的真实节点，初始化渲染完成
